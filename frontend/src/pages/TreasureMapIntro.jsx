...
const TreasureMapIntro = () => {
    const navigate = useNavigate();

    const FEATURES = [
        { title: 'Приключение', desc: 'Открой все секретные точки на карте', icon: IconMap, color: 'text-yellow-600' },
        { title: 'Награды', desc: 'Находи сундуки с ценными призами', icon: IconTrophy, color: 'text-amber-500' },
        { title: 'Прогресс', desc: 'Твой путь сохраняется автоматически', icon: IconStar, color: 'text-orange-500' },
        { title: 'Цель', desc: 'Доберись до главного сокровища', icon: IconHeart, color: 'text-red-500' },
    ];
    ...
    return (
        <div className="bg-amber-50 min-h-screen flex flex-col items-center relative overflow-hidden font-sans">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
                <div className="absolute top-10 left-10 w-64 h-64 bg-yellow-300 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
                ...
                <div className="absolute bottom-10 left-1/3 w-80 h-80 bg-orange-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000"></div>
            </div>
            ...
            <div className="w-full pt-8 pb-4 flex flex-col items-center z-20 relative">
                <motion.a
                    href="/"
                    className="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-600 to-amber-500 font-sans tracking-tighter cursor-pointer"
            ...
            <div className="max-w-4xl w-full z-10 flex flex-col md:flex-row gap-8 items-center justify-center p-4">
                ...
                >
                    ...
                    <p className="text-slate-600 text-center mb-12 max-w-xs text-sm font-medium leading-relaxed">
                        Стань легендарным искателем приключений! Прокладывай путь по карте, выполняй задания и находи скрытые сокровища.
                    </p>

                    <motion.button
                        whileHover={{ scale: 1.05, translateY: -2 }}
                        whileTap={{ scale: 0.95 }}
                        onClick={() => navigate('/game/treasure/play')}
                        className="w-full py-5 rounded-2xl font-black text-white text-xl shadow-xl
                            ...
                            bg-gradient-to-r from-yellow-500 to-amber-700 hover:shadow-yellow-200"
                    >
                        🎮 В ПУТЬ
                    </motion.button>
                </motion.div>
...
export default TreasureMapIntro;
