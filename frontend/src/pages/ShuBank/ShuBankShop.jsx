...
const SHOP_ITEMS = [
    ...
    { id: 'item_mining_farm', name: 'Майнинг-ферма', price: 5000, category: 'item', icon: '📟' },
];

const ShuBankShop = () => {
    const navigate = useNavigate();
    const { stats, loading, error, fetchState, buyItem, equipItem } = useShuBank();
    ...
    const handleBuy = async (item) => {
        ...
        if (res.success) {
            ...
            // Automatically equip after buy? 10/10 feature
            await equipItem(item.id);
        } else {
            showMessageFn(`Ошибка: ${res.error || "Не удалось купить"}`);
        }
    ...
    const handleEquip = async (item) => {
        ...
        if (res.success) {
            showMessageFn(`${item.name} надет! ✨`);
        }
        isActionPendingRef.current = false;
    };

    return (
        <GameLoader loading={loading || !stats} error={error} retry={fetchState} message="Загрузка магазина...">
            <div className="h-screen bg-slate-950 flex justify-center">
                ...
                >
                    ...
                    <img src={backgroundPng} className="hidden" loading="lazy" alt="preload" />
                    {SHOP_ITEMS.map(item => (
                        <img key={item.id} src={item.icon} className="hidden" loading="lazy" alt="preload" />
                    ))}
                    ...
                    <main className="flex-1 overflow-y-auto px-6 pt-24 pb-12 scrollbar-hide">
                        <div className="max-w-md mx-auto">
                            <h2 className="text-3xl font-black italic uppercase tracking-tighter text-white mb-8 drop-shadow-lg font-montserrat">Магазин Образов</h2>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {SHOP_ITEMS.map(item => {
                                    const isOwned = stats?.inventory?.includes(item.id) || false;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-1 text-orange-500 font-black text-[11px] mb-4 font-montserrat">
                                                {isOwned ? (
                                                    <span className="text-green-600 uppercase font-montserrat">В коллекции</span>
                                                ) : (
                                                    <>
                                                        <span className="font-montserrat">{item.price}</span>
                                            ...
                                            {isOwned ? (
                                                <button
                                                    ...
                                                    disabled={stats?.current_skin_id === item.id}
                                                    className={`w-full py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${stats?.current_skin_id === item.id
                                                        ? 'bg-green-500 text-white cursor-default'
                                                        : 'bg-blue-600 text-white hover:scale-105 active:scale-95'
                                                        }`}
                                                >
                                                    {stats?.current_skin_id === item.id ? 'Надето' : 'Надеть'}
                                                </button>
                                            ) : (
                                                <button
                                                    onClick={() => handleBuy(item)}
                                                    disabled={!canAfford}
                                                    className={`w-full py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${canAfford
