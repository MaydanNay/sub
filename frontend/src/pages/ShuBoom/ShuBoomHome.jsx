...
import { getCurrentCharacter, getNextCharacter } from './gameData';
...
const ShuBoomHome = () => {
    ...
    // Derived state
    const currentChar = user ? getCurrentCharacter(user.current_status) : null;
    const sublevel = user ? (user.current_sublevel || 0) : 0;

    const handleDailyReward = async () => {
        try {
            const res = await axios.post(`${API_URL}/user/daily_reward`, null, { params: { user_phone: USER_PHONE } });
            if (res.data.success) {
                fetchUser();
    ...
    return (
        <div className="h-screen w-full overflow-hidden flex flex-col bg-slate-900 font-sans">
                ...
                <div className="pt-6 px-6 relative z-10 transition-all duration-300">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <div className="text-white/80 text-[11px] font-bold uppercase tracking-wider mb-0.5">Твой Статус</div>
                            <h1 className="text-3xl font-black text-white leading-tight drop-shadow-md">{currentChar?.name}</h1>
            ...
            <div className="relative z-10 flex-1 -mt-10 pt-14 pb-24 bg-slate-900 flex flex-col">
                ...
                <div className="absolute inset-0 w-full h-full z-0">
                    <img src={localCoffeeBg} alt="Background" className="w-full h-full object-cover opacity-80" />
                ...
                <div className="px-6 relative z-10 flex flex-col h-full w-full">
                    ...
                    >
                        <div className="flex justify-between items-center mb-3">
                            <div>
                                ...
                                <p className="text-[10px] text-white/70 font-bold">
                                    Серия: <span className="text-white">{user.daily_streak} дн.</span>
                                </p>
                            </div>
                            {(() => {
                        ...
                        <div className="flex justify-between gap-1">
                            {[1, 2, 3, 4, 5, 6, 7].map(day => {
                                const active = (user.daily_streak % 7) + 1 === day;
                                const done = (user.daily_streak % 7) + 1 > day;
                                return (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                <h3 className="font-bold text-gray-900 text-sm">Задания на день</h3>
                                <p className="text-[10px] text-gray-600 font-medium">Выполняй и получай монеты</p>
                            </div>
                        </div>
                    ...
                    <div className="flex justify-center items-end mt-auto mb-4 relative flex-1 min-h-0">
                        ...
                        <motion.img
                            ...
                            className="h-full max-h-[40vh] w-auto object-contain filter drop-shadow-2xl z-20"
                        />
                        {/* Glow effect */}
                    ...
                    infoOpen === 'bonus' && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm" onClick={() => setInfoOpen(false)}>
                            ...
                            >
                                ...
                                <h2 className="text-2xl font-black text-gray-800 mb-2">Ежедневный Бонус</h2>
                                <p className="text-gray-500 font-medium mb-8">Заходи каждый день, чтобы увеличивать свою серию и получать больше монет!</p>

                                <div className="grid grid-cols-4 gap-3 mb-8">
                                    {[1, 2, 3, 4, 5, 6, 7, '∞'].map((d, i) => (
                                        <div key={i} className={`p-2 rounded-2xl border-2 ${user.daily_streak % 7 > i ? 'border-green-400 bg-green-50' : 'border-slate-100'}`}>
                                            <div className="text-[10px] font-bold text-slate-400 uppercase">День</div>
                                            <div className="font-black text-gray-800">{d}</div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-6 bg-black/60 backdrop-blur-sm" onClick={() => setInfoOpen(false)}>
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between mb-6">
                                    <h2 className="text-2xl font-black text-gray-800">Задания</h2>
                                    <div className="text-xs font-bold text-slate-400 bg-slate-200 px-2 py-1 rounded-lg">Обновится через 4ч</div>
                                </div>

                                <div className="space-y-3 overflow-y-auto pr-1">
                                    {[
                                        { title: "Кофейный Барон", desc: "Купи 3 капучино", icon: "☕", progress: 0, max: 3, reward: 200, color: "text-amber-600 bg-amber-50" },
                                        { title: "Сладкая Жизнь", desc: "Закажи десерт", icon: "🍰", progress: 0, max: 1, reward: 150, color: "text-pink-600 bg-pink-50" },
                                        { title: "Дружба", desc: "Пригласи друга", icon: "🤝", progress: 0, max: 1, reward: 500, completed: false, color: "text-blue-600 bg-blue-50" }
                                    ].map((quest, i) => (
                                        <div
                                            key={i}
                                            className={`p-4 rounded-3xl border border-slate-200 flex items-center gap-4 ${quest.completed ? 'bg-slate-100 opacity-60' : 'bg-white shadow-sm'}`}
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start mb-1">
                                                    <h4 className={`font-bold text-sm ${quest.completed ? 'line-through text-gray-400' : 'text-gray-800'}`}>{quest.title}</h4>
                                                    {quest.completed ?
                                                        <span className="text-[10px] font-bold text-green-500 bg-green-50 px-2 py-0.5 rounded-md">Выполнено</span> :
                                                ...
                                                <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden mt-1">
                                                    <div
                                                        ...
                                                        className={`h-full rounded-full ${quest.completed ? 'bg-green-400' : 'bg-indigo-500'}`}
                                                    />
                                                </div>
                                                <div className="text-[10px] text-gray-400 mt-1 font-medium">{quest.desc} • {quest.progress}/{quest.max}</div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
