...
const DiceIntro = () => {
    ...
    const FEATURES = [
        { title: 'Удача', desc: 'Проверь свою везучесть сегодня', icon: IconStar, color: 'text-pink-500' },
        { title: 'Награды', desc: 'Мгновенные купоны и бонусы', icon: IconTrophy, color: 'text-purple-500' },
        { title: 'Азарт', desc: 'Испытай яркие эмоции от игры', icon: IconHeart, color: 'text-red-500' },
        { title: 'Простота', desc: 'Один бросок отделяет от приза', icon: IconMap, color: 'text-indigo-500' },
    ];

    return (
        <div className="bg-pink-50 min-h-screen flex flex-col items-center relative overflow-hidden font-sans">
            ...
            <div className="max-w-4xl w-full z-10 flex flex-col md:flex-row gap-8 items-center justify-center p-4">
                ...
                >
                    ...
                    <p className="text-slate-600 text-center mb-12 max-w-xs text-sm font-medium leading-relaxed">
                        Брось кубик и испытай удачу! Каждый бросок может принести тебе уникальную скидку или подарок.
                    </p>

                    <motion.button
                        whileHover={{ scale: 1.05, translateY: -2 }}
                        whileTap={{ scale: 0.95 }}
                        onClick={() => navigate('/game/dice/play')}
